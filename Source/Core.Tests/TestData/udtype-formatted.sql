...
CREATE OR ALTER PROCEDURE [Measurement].[uspSaveMeasurementValues] @pPollRequestId     BIGINT
...
*/
        ...
        SET @ContextMessage = 'Begin inserting measurement Values for PollRequestId = ' + CONVERT(VARCHAR(100),@pPollRequestId) + ' and PollResponseId = ' + CONVERT(VARCHAR,@pPollResponseId);

        BEGIN TRY
            BEGIN TRANSACTION;
            INSERT INTO Measurement.MeasurementValue(PollResponseValueId
                   ...
                   SELECT PollResponseValue.PollResponseValueId
                     ...
                     FROM @pMeasurementValues AS response
                          INNER JOIN Measurement.PollRequest AS PollRequest
                                  ON PollRequest.PollRequestId = @pPollRequestId
                          INNER JOIN Measurement.DevicePollRequest AS DevicePollRequest
